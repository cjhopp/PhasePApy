...
class Plot:
    ...
    def event_plot(self, assoc_id, west=-104.5, east=-94, south=33.5,
        ...
        for match in matches:
            lon, lat = self.tt_stations_db_1D.query(Station1D.longitude,
                                                    Station1D.latitude).filter(
    ...
    def distance_singlephase(self, phase, time):
        ...
        """
        if phase == 'P':
            min = self.tt_stations_db_1D.query(TTtable1D).filter(
                TTtable1D.p_tt <= time).order_by(TTtable1D.p_tt.desc()).first()
            max = self.tt_stations_db_1D.query(TTtable1D).filter(
                TTtable1D.p_tt >= time).order_by(TTtable1D.p_tt).first()
        ...
        if phase == 'S':
            min = self.tt_stations_db_1D.query(TTtable1D).filter(
                TTtable1D.s_tt <= time).order_by(TTtable1D.s_tt.desc()).first()
            max = self.tt_stations_db_1D.query(TTtable1D).filter(
                TTtable1D.s_tt >= time).order_by(TTtable1D.s_tt).first()
    ...
    def section_plot(self, assoc_id, files, seconds_ahead=5, record_length=100,
        ...
        station = self.assoc_db.query(Candidate.sta).filter(
            Candidate.assoc_id == assoc_id).all()
        sta_list = []
        for sta, in station:
        ...
        while True:
            ...
            if not duplicate:
                break
            index = [i for (i, j) in enumerate(ST_new_sta) if
                     j == duplicate[-1]]
            i = 0
            while True:
                if ST_new[index[i]].stats.npts < ST_new[
                    index[i + 1]].stats.npts:
                    del ST_new[index[i]]
                    break
                elif ST_new[index[i]].stats.npts >= ST_new[
                    index[i + 1]].stats.npts:
        ...
        for tr in ST_new:
            ...
            if channel == 'Z3':
                picks_p = self.assoc_db.query(Pick.time).filter(
                    Pick.assoc_id == assoc_id).filter(
