...
class Pick(Base):
    ...
    def __repr__(self):
        return "Pick <%s.%s.%s.%s %s %s %s %s>" % (
            ...
            self.phase, self.modified_id, self.assoc_id)
...
class Candidate(Base):
    ...
    # calculated but simpler to save
    tp = Column(DateTime)
...
class Associated(Base):
    ...
    def __init__(self, ot, ot_uncert, latitude, longitude, loc_uncert, nsta,
                 t_create, t_update):
        self.ot = ot
    ...
    def __repr__(self):
        return "Associated Event <%s %s %.3f %.3f %d>" % (
            self.ot.isoformat("T"), self.ot, self.latitude, self.longitude,
            self.nsta)
