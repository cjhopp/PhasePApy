...
from .cf_kt import rms, rolling_window, Kurtosis
...
class KTPicker:
    ...
    def __init__(self, t_win=1, t_ma=10, nsigma=6, t_up=0.2, nr_len=2,
        ...
        t_up: float
            the time in seconds not allowed consecutive pick in this duration
        nr_len: float
            noise ratio filter window length before and after potential
            picks used to calculate standard deviation
        nr_coeff: float
            control threshold level to determine if remove the pick by
            comparing std or rms on both sides of each potential pick
        pol_len: float
            window length in samples to calculate the standard deviation
            of waveform before the picks
        pol_coeff: float
            determine if declare first motion as 'Compression' or 'Dilation'
            by comparing the first local extreme value after pick and standard
            deviation in previous window
        uncert_len: float
            window length in time to calculate the rms of the CF before the
            picks, we make it as long as t_ma
        uncert_coeff: float
...
class KTSummary():
    ...
    def pick_ident(self):
        ...
        for i in range(0, len(trigger_ptnl) - 1):  # second from last
            if (trigger_ptnl[i + 1] - trigger_ptnl[
                i]) <= window_t_up * dt:  # avoid consecutive picking
    ...
    def uncertainty(self):
        ...
        for i in range(len(trigger)):
            ...
            while True:
                if self.summary[index] > uncert_level and self.summary[index] > \
    ...
    def winlen(self, index, trigger_ptnl, filter_length, t, dt):
        """
        Determine the filter window length. If the time difference between two
        picks is less than window length, use the picks interval as window.
        """
        i = index
