...
class AICDPicker:
    ...
    def __init__(self, t_ma=3, nsigma=6, t_up=0.2, nr_len=2,
        ...
        Parameters
        ----------
        t_ma: the time in seconds of the moving average window for dynamic
            threshold
        ...
        nr_len: noise ratio filter window length before and after potential
            picks used to calculate standard deviation
        nr_coeff: control threshold level to determine if remove the pick by
            comparing std or rms on both sides of each potential pick
        pol_len: window length in samples to calculate the standard deviation
            of waveform before the picks
        pol_coeff: determine if declare first motion as 'Compression' or
            'Dilation' by comparing the first local extreme value after pick
            and standard deviation in previous window
        uncert_len: window length in time to calculate the rms of the CF
            before the picks, we make it as long as t_ma
...
class AICDSummary:
    """
    The class calculate CF, threshold level, cleans the false picks,
    determines uncertainty, polarity and plot CF.
    """
    ...
    def threshold(self):
        ...
        threshold[npts_Tma:LEN] = rms(
            rolling_window(self.summary[0:LEN - 1], npts_Tma),
    ...
    def pick_ident(self):
        ...
        scnl = SCNL([self.stats.station, self.stats.channel, self.stats.network,
                     self.stats.location])
        dt = self.stats.delta
        npts_Tma = int(round(self.picker.t_ma / dt, 0))
        LEN = self.stats.npts
        ...
        for i in range(len(trigger_ptnl)):
            ...
            if N * np.std(self.tr.data[
                          int(round(trigger_ptnl[i] / dt, 0)) - M:int(
                              round(trigger_ptnl[i] / dt, 0))]) >= np.std(
                self.tr[int(round(trigger_ptnl[i] / dt, 0)):int(
                    round(trigger_ptnl[i] / dt, 0)) + M]):
        ...
        # really need to be careful copy list or array,
        # since if just copy like A=B, when any element in B changes,
        ...
        # really need to be careful copy list or array, since if just copy
    ...
    def uncertainty(self):
        ...
        for i in range(len(trigger)):
            ...
            while True:
                if self.summary[index] > uncert_level \
                        and self.summary[index] > \
                        self.summary[index - 1]:
                    index -= 1
                else:
                    break
            pick_uncert[i] = round(
                t[index] - (picks[i] - self.tr.stats.starttime), 3)
    ...
    def polarity(self):
        ...
        for i in range(len(picks)):
            ...
            # notice index+1, rolling stop one point before extreme,
            # compare with std to avoid very small
            if self.tr[index + 1] - self.tr[index0] > 0 and abs(
                            self.tr[index + 1] - self.tr[
                        index0]) > self.picker.pol_coeff * np.std(
    ...
    def winlen(self, index, trigger_ptnl, filter_length, t, dt):
        """
        Determine the filter window length. If the time difference between
         two picks is less than window length, use the picks interval
         as window.
        """
        i = index
        if len(trigger_ptnl) == 1:
            ...
            if i == 0:
                ...
                if trigger_ptnl[i] + filter_length > t[-1]:
                    R = int(
                        round((t[-1] - trigger_ptnl[i]) / dt, 0));  # print 'k'
                if trigger_ptnl[i] + filter_length <= t[-1]:
    ...
    def plot_summary(self):
        ...
        ax2.plot(t, self.thres / np.amax(self.summary), '--', linewidth=2.0,
                 c='k')
        ...
        # Plot picks
        # for i in range(len(picks)):
        #  ax2.plot([(picks[i]-self.tr.stats.starttime),
        # (picks[i]-self.tr.stats.starttime)],[0,1],'r--')
        #  ax2.text((picks[i]-self.tr.stats.starttime),
        # 0.5,'%s' % (self.pol[i]),color='red')
        ax2.legend(('Normalized CF', 'Threshold', 'Picks'), 'upper right',
